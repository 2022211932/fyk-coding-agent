...
Yukai-SE 把软件工程规范落实为后端可校验的状态机，而不是只在提示词里要求模型“按流程做”：

1. **需求分析 Skill**：建立 `FR-001` / `NFR-001` 需求和可验证的验收标准；
2. **结构化设计 Skill**：用 `MOD-001` 模块描述职责、接口，并覆盖全部需求；
3. **结构化实现 Skill**：需求到实现文件的映射必须引用成功的写文件或编辑证据；
4. **测试验证 Skill**：需求到测试命令的映射必须引用成功的验证命令证据；
5. **验收与变更控制 Skill**：测试覆盖完整后，由用户做最终验收决策。

从需求进入设计前必须由用户确认需求基线，最终完成前必须由用户确认项目验收。对于无法从仓库发现、且会实质影响范围、架构、约束或验收标准的信息，Agent 会调用内部 `request_user_input` 工具暂停执行。Web 界面显示决策卡片，命令行显示编号选项；回答被记录后会恢复原流程。普通实现细节仍由 Agent 自行检查和判断，避免频繁打断用户。
