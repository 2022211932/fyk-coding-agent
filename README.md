...
从需求进入设计前必须由用户确认需求基线，最终完成前必须由用户确认项目验收。对于无法从仓库发现、且会实质影响范围、架构、约束或验收标准的信息，Agent 会调用内部 `request_user_input` 工具暂停执行。Web 界面显示决策卡片，命令行显示编号选项；回答被记录后会从持久化工程证据继续原流程。需求确认、设计覆盖、实现覆盖和验证覆盖通过后会自动进入下一阶段，避免模型因手动推进顺序产生无意义失败。

验收卡会汇总需求数、模块数、实现/验证追踪和过期证据，并明确剩余风险边界。`complete_project` 成功后 Agent 立即结束本轮，剩余工具调用会被跳过；已验收项目默认只读，只有先将生命周期退回受影响阶段、形成变更请求后才能继续修改或执行命令。
