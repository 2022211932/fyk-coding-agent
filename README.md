...
从需求进入设计前必须由用户确认需求基线，最终完成前必须由用户确认项目验收。对于无法从仓库发现、且会实质影响范围、架构、约束或验收标准的信息，Agent 会调用内部 `request_user_input` 工具暂停执行。需求基线卡会展示每项 FR/NFR 的标题、描述、全部验收标准以及待确认的默认决策，而不是只给出需求编号；确认结果绑定当前需求摘要，需求变化后旧确认卡自动失效。回答被记录后会从持久化工程证据继续原流程。需求确认、设计覆盖、实现覆盖和验证覆盖通过后会自动进入下一阶段，避免模型因手动推进顺序产生无意义失败。
