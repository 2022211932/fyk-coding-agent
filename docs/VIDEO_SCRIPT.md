...
建议录屏分辨率 1920x1080。提前安装项目、前端依赖并设置环境变量，视频中不要打开 `.env`、控制台密钥页或环境变量详情。
...
```powershell
fyk-agent --web -w demo-workspace
```
...
> 这是 FYK Agent 的可视化控制台，多轮指令共享上下文。中央时间线实时展示模型和工具的协作过程，右侧同步展示文件变更。模型只能提出结构化工具调用，本地工具先检查路径和参数；写入和命令得到页面审批后才会执行。
